...
def run_recbole(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True,pretrained_file=None):
    ...
    os.popen(f'cp {config["data_path"]}/{config["dataset"]}.kg.{config["kg_name"]} {config["data_path"]}/{config["dataset"]}.kg')
    ...
    train_data = KnowledgeBasedDataLoader(config, train_dataset, train_sampler, kg_sampler, shuffle=True)
