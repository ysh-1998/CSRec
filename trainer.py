...
class MultiDataTrainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        super(MultiDataTrainer, self).__init__(config, model)
    ...
    def _train_epoch(self, train_data, epoch_idx, train_data_=None, loss_func=None, show_progress=False):
        ...
        if train_data_ is None:
            ...
            iter_data = (
                tqdm(
                    zip(train_data,train_data_),
                    total=len(train_data),
                    ncols=100,
                    desc=set_color(f"Train {epoch_idx:>5}", 'pink'),
                ) if show_progress else train_data
            )
            for batch_idx, (interaction,interaction_) in enumerate(iter_data):
                interaction = interaction.to(self.device)
                interaction_ = interaction_.to(self.device)
                self.optimizer.zero_grad()
                losses = loss_func(interaction,interaction_)
                if isinstance(losses, tuple):
                    loss = sum(losses)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
                ...
                if self.gpu_available and show_progress:
                    iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
                if train_data_.kg_dataloader.pr_end - train_data_.kg_dataloader.pr < self.config["train_batch_size"]:
                    train_data_.kg_dataloader.pr = 0
            train_data_.general_dataloader.pr = 0
            train_data_.kg_dataloader.pr = 0
        return total_loss
    ...
    def fit(self, train_data, train_data_, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        if self.config['train_neg_sample_args'].get('dynamic', 'none') != 'none':
            ...
            train_data_.get_model(self.model)
        valid_step = 0

        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss = self._train_epoch(train_data, epoch_idx, train_data_, show_progress=show_progress)
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            # _, topk_idx = torch.topk(scores, 10, dim=-1)
            # hit = 0
            # with open(f"csrec_{self.config['model']}_hit_result.txt", "w") as f:
            #     for u,i in zip(positive_u, positive_i):
            #         u_id, i_id = eval_data.dataset.field2id_token["user_id"][u+1], eval_data.dataset.field2id_token["item_id"][i]
            #         f.write(str(u_id)+"\t"+str(i_id)+"\t")
            #         if u in torch.where(topk_idx==i)[0]:
            #             hit += 1
            #             f.write("1\n")
            #         else:
            #             f.write("0\n")
            # print("hit:",hit/len(positive_u))
...
class MultiKGTrainer(MultiDataTrainer):
    ...
    def __init__(self, config, model):
        super(MultiKGTrainer, self).__init__(config, model)

        self.train_rec_step = config['train_rec_step']
        self.train_kg_step = config['train_kg_step']

    def _train_epoch(self, train_data, epoch_idx, train_data_, loss_func=None, show_progress=False):
        ...
        train_data_.set_mode(interaction_state)
        if interaction_state in [KGDataLoaderState.RSKG, KGDataLoaderState.RS]:
            return super()._train_epoch(train_data, epoch_idx, train_data_, show_progress=show_progress)
        elif interaction_state in [KGDataLoaderState.KG]:
            return super()._train_epoch(
                train_data, epoch_idx, train_data_, loss_func=self.model.calculate_kg_loss, show_progress=show_progress
            )
        return None